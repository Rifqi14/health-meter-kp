    ...
    <div class="col-lg-4">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title">Detail Medis</h3>
            ...
            <div class="box-body box-profile">
                <ul class="list-group list-group-unbordered">
                    <li class="list-group-item">
                        <b>Nama</b> <span class="pull-right">{{$medical->name}}</span>
                    </li>
                    <li class="list-group-item">
                        <b>Tipe</b> <span class="pull-right">{{$type[$medical->type]}}</span>
                    </li>
                </ul>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Tambah Medis Detail</h4>
            </div>
            <div class="modal-body">
                <form id="form" method="post" action="{{route('medicaldetail.store')}}" autocomplete="off">
                    {{ csrf_field() }}
...
<script>
    function adddetail() {
        ...
        $('#form').attr('action', '{{route('medicaldetail.store')}}');
    ...
    $(document).ready(function () {
        ...
        dataTable = $('#table-detail').DataTable({
            ...
            ajax: {
                url: "{{url('admin/medicaldetail/read')}}",
                type: "GET",
                data: function (data) {
                    data.medical_id = {{$medical->id}};
                }
            },
            columnDefs: [{
                    orderable: false,
        ...
        $(document).on('click', '.edit', function () {
            var id = $(this).data('id');
            $.ajax({
                url: `{{url('admin/medicaldetail')}}/${id}/edit`,
                method: 'GET',
                dataType: 'json',
                beforeSend: function () {
                    $('#medicaldetail .overlay').removeClass('hidden');
                ...
                if (response.status) {
                    ...
                    $('#form').attr('action',
                        `{{url('admin/medicaldetail/')}}/${response.data.id}`);
                }
            }).fail(function (response) {
                var response = response.responseJSON;
                $('#medicaldetail .overlay').addClass('hidden');
            ...
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result) {
                        ...
                        $.ajax({
                            url: `{{url('admin/medicaldetail')}}/${id}`,
                            dataType: 'json',
                            data: data,
                            type: 'DELETE',
                            beforeSend: function () {
