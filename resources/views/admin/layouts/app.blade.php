                  ...
                  <ul class="nav navbar-nav">
                    ...
                    <li class="dropdown user user-menu">
                      <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                        <img src="{{is_file('assets/user/'.Auth::guard('admin')->user()->id.'.png')?asset('assets/user/'.Auth::guard('admin')->user()->id.'.png'):asset('adminlte/images/user2-160x160.jpg')}}" class="user-image" alt="User Image">
                        <span class="hidden-xs">{{ Auth::guard('admin')->user()->name }}</span>
                      </a>
                      <ul class="dropdown-menu">
                        <li class="user-header">
                          <img src="{{is_file('assets/user/'.Auth::guard('admin')->user()->id.'.png')?asset('assets/user/'.Auth::guard('admin')->user()->id.'.png'):asset('adminlte/images/user2-160x160.jpg')}}" class="img-circle" alt="User Image">
                          <p>
          ...
          <div class="modal-dialog">
              <div class="modal-content">
                  ...
                  <div class="modal-body">
                      ...
                      @foreach(Auth::guard('admin')->user()->sites()->get() as $site)
                          <li class="list-group-item ">
                              <a href="{{url('admin/site/set/'.$site->id)}}" class="text-muted font-bold"><strong>{{$site->name}}</strong></a>
                          </li>
                          @endforeach
                      </ul>
                  </div>
                  <div class="modal-footer">
                      <a href="{{ route('admin.logout') }}"
                      onclick="event.preventDefault();
                                    document.getElementById('logout-form').submit();" class="btn btn-primary" ><i class="fa fa-sign-out"></i></a>
                  </div>
              </div>
          </div>
      </div>
