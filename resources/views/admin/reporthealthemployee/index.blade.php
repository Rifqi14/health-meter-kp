...
@endsection
@push('breadcrump')
    <li class="active">Laporan Kesehatan Karyawan</li>
@endpush
@section('content')
...
<div class="row">
    ...
    <div class="box box-primary">
        <div class="box-header">
            <h3 class="box-title">Laporan Kesehatan Karyawan</h3>
...
$(function(){
    ...
    $('#form-export .date-picker').daterangepicker({
		...
		if(picker.endDate.diff(picker.startDate, 'days') > 30){
				...
				$('#form-export').find('input[name=order_date]').data('daterangepicker').setStartDate(moment(new Date().setHours(0,0)).subtract(6, 'days'));
	            $('#form-export').find('input[name=order_date]').data('daterangepicker').setEndDate(moment(new Date().setHours(23,59)));
				return false;
			}
        else{
    ...
    dataTable = $('.datatable').DataTable( {
        ...
        order: [[ 1, "desc" ]],
        ajax: {
            url: "{{route('reporthealthemployee.read')}}",
            type: "GET",
            data:function(data){
    ...
    $('#form-export').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: "{{ route('reporthealthemployee.export') }}",
            type: 'POST',
            dataType: 'JSON',
